...
export default function ApplicantsDashboard() {
  ...
  const interviewStatusOptions = [
    "All",
    ...Array.from(
      new Set(applicants.map((a) => a.interviewStatus).filter(Boolean)),
    ),
  ];
  ...
  async function fetchApplications(p = page) {
    ...
    try {
      ...
      if (applicationStatusFilter)
        params.append("applicationStatus", applicationStatusFilter);
      if (interviewStatusFilter)
        params.append("interviewStatus", interviewStatusFilter);
  ...
  return (
    <div className="p-8 px-[15em] justify-center center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 p-4 border-2 border-[#e0e0e0] dark:border-[#3a3f55] rounded-lg">
        <div>
          <label
            htmlFor="nameSearch"
